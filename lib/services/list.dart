...
class ListService {
  Future<void> saveItem({uid, list, item: String}) {
    // run a transaction for this https://firebase.google.com/docs/firestore/manage-data/transactions
    DocumentReference uidDocumentRef =
        Firestore.instance.collection('lists').document(uid);
    return Firestore.instance.runTransaction(
      (Transaction t) =>
          t.get(uidDocumentRef).then((DocumentSnapshot uidDocument) async {
        List currentLists = [];
  ...
  Future<void> removeAt({List<dynamic> list, index: int, uid: String}) async {
    ...
    await Firestore.instance
        .collection('lists')
        .document(uid)
        .setData(<String, dynamic>{'lists': list});
  }
  ...
  Stream<List> getUserLists({uid: String}) {
    return Firestore.instance
        ...
        .document(uid)
        .snapshots()
        .map((v) => v.data['lists'] as List);
  }

  // this is how you get data out of a stream, you maybe have to unsubscribe?
  ...
  Future<String> getItemsFromList({list, uid: String}) async {
    ...
    var itemsInList = await Firestore.instance
        .collection('lists')
        .document(uid)
        .collection(list)
        .getDocuments();
